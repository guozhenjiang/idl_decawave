...
void setup_DW1000RSTnIRQ(int enable)
{
    ...
    if(enable)
    {
        ...
        HAL_NVIC_EnableIRQ((IRQn_Type)EXTI0_IRQn);     //pin #0 -> EXTI #0
        HAL_NVIC_SetPriority((IRQn_Type)EXTI0_IRQn, 5, 0);
    }
    else
    {
        HAL_NVIC_DisableIRQ((IRQn_Type)EXTI0_IRQn);    //pin #0 -> EXTI #0
...
{
    NVIC_DisableIRQ((IRQn_Type)DECAIRQ_EXTI_IRQn);
}
...
{
    NVIC_EnableIRQ((IRQn_Type)DECAIRQ_EXTI_IRQn);
}
